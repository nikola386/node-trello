...
var Trello = module.exports = function (opts) {
  ...
  if (opts.useOAuth && (!opts.accessToken || !opts.accessTokenSecret || !opts.appApiSecret)) {
    throw new Error("Access token and secret are required");
  } else if (!opts.useOAuth && (!opts.appApiKey || !opts.userToken)) {
  ...
  if(this.opts.useOAuth) {
    this.appApiSecret = opts.appApiSecret;
...
Trello.prototype.request = function (method, uri, argsOrCallback, callback) {
  ...
  request[method === 'DELETE' ? 'del' : method.toLowerCase()](options, function (err, response, body) {
    const retryIn = response.headers['x-rate-limit-api-key-interval-ms'];
    const apiKeyMax = response.headers['x-rate-limit-api-key-max'];
    const apiKeyRemaining = response.headers['x-rate-limit-api-key-remaining'];
    const apiTokenMax = response.headers['x-rate-limit-api-token-max'];
    const apiTokenRemaining = response.headers['x-rate-limit-api-token-remaining'];

    if(apiKeyRemaining <= 298 || apiTokenRemaining <= 298) {
      return setTimeout(this.request.call(this), retryIn);
    }

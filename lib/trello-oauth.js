...
var TrelloOAuth = module.exports = function (key, secret, loginCallback, rights, appName) {
...
TrelloOAuth.prototype.getRequestToken = function (callback) {
  var appName = this.appName, rights = this.rights;
  ...
  this.oauth.getOAuthRequestToken(function (error, token, tokenSecret, results) {
    ...
    callback(null, {
      ...
      redirect: authorizeUrl + ("?oauth_token=" + token + "&expiration=never&scope=" + rights + "&name=" + appName)
    });
  });
};
